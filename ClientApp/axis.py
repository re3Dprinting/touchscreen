...
#		But the soft limits should be on the firmware level. 
	...
	def buttonreleased(self):
		self.timer.stop()
		if (self.button_held_time == 0 or self.holdmove == None) and self.direction == "Pos": self.movepos()
		elif (self.button_held_time == 0 or self.holdmove == None) and self.direction == "Neg": self.moveneg()
		self.button_held_time = 0
	def button_event_check(self):
		self.button_held_time += 0.25
		if self.holdmove != None:
			if self.direction == "Pos": self.movepos(self.holdmove)
			elif self.direction == "Neg": self.moveneg(self.holdmove)
	...
	def movepos(self, inc = None):
		if inc == None: inc = self.inc
		self.parent.serial.send_serial('G91')
		self.parent.serial.send_serial('G1 '+ self.Ax+ str(inc) + ' F'+ self.feedrate)

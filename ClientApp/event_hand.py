...
class event_handler(QtCore.QThread):
    ...
    def sendflowrate(self):
        if self.fr_index == 0:
            self.tempwindow.serial.send_serial(
                "M221 S" + str(self.flowrate[self.fr_index]) + " T0")
            self.tempwindow.serial.send_serial(
                "M221 S" + str(self.flowrate[self.fr_index]) + " T1")
            ...
            if self.fr_index == 1:
                ...
                t = " T1"
            self.tempwindow.serial.send_serial(
                "M221 S" + str(self.flowrate[self.fr_index]) + t)

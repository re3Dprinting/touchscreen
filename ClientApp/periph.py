...
class Periph(object):
	...
	def init_posneg(self):
		getattr(self.parent, self.periph + "pos").clicked.connect(self.increment)
		getattr(self.parent, self.periph + "neg").clicked.connect(self.decrement)

		getattr(self.parent, self.periph + "pos").pressed.connect(self.incrementbuttonpressed)
		getattr(self.parent, self.periph + "neg").pressed.connect(self.decrementbuttonpressed)

		getattr(self.parent, self.periph + "pos").released.connect(self.buttonreleased)
		getattr(self.parent, self.periph + "neg").released.connect(self.buttonreleased)
	...
	def buttonreleased(self):
		self.timer.stop()
		if (self.button_held_time == 0 and self.direction == "inc"): self.increment()
		elif (self.button_held_time == 0 and self.direction == "dec"): self.decrement()
		self.sendtemp()
		self.button_held_time = 0
	def button_event_check(self):
		...
		if self.button_held_time >= 1:
			if self.direction == "inc": self.increment()
